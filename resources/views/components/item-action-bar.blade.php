<div class="action-container">
    @foreach ($template as $key => $item)
        @if ($key !== 'delete')
            ...
            @else
                <a href="{{ route($controller . 's.' . $item['route'], [$controller => $id]) }}"
                    class="{{ $item['class'] }}" data-toggle="tooltip" title="{{ $item['content'] }}">
                    <i class="{{ $item['icon'] }}"></i>
                </a>
            @endif
        @else
            <form class="delete-item-form" action="{{ route($controller . 's.destroy', [$controller => $id]) }}"
                method="post" data-item-name="{{ $name }}">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
